...
export default function EditProductPage() {
    ...
    const [formData, setFormData] = useState({
        name: '', description: '', store_id: '', brand_id: '',
        categories: [], status: 'draft', main_status: 'unavailable',
        images: [], imagePreviews: [], existingImages: [],
        has_variants: false, price: '', stock: '',
        tier_prices: [], selected_options: [], variants: [],
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [storesRes, brandsRes, catsRes, optionsRes, productRes] = await Promise.all([
                api.getStores({ limit: 100 }), api.getBrands(), api.getCategories(), api.getOptions(), api.getProduct(productId)
            ]);
            ...
            if (optionsRes.success) setOptions(allOptions);

            if (productRes.success) {
                const p = productRes.data;
                const isSimple = !p.has_variants || p.has_variants === 0;
                const simpleVariant = isSimple && p.variants?.length > 0 ? p.variants[0] : null;

                const valueToOptionMap = {};
                allOptions.forEach(opt => { opt.values?.forEach(val => { valueToOptionMap[val.id] = opt.id; }); });

                setFormData(prev => ({
                    ...prev,
                    name: p.name, description: p.description || '',
                    store_id: p.store?.id || p.store_id || '',
                    brand_id: p.brand?.id || p.brand_id || '',
                    categories: p.categories?.map(c => c.id) || [],
                    status: p.status, main_status: p.main_status || 'available',
                    meta_title: p.seo?.meta_title || p.meta_title || '',
                    meta_description: p.seo?.meta_description || p.meta_description || '',
                    keywords: p.seo?.keywords || p.keywords || '',
                    has_variants: p.has_variants === 1 || p.has_variants === true,
                    price: simpleVariant ? simpleVariant.price : '',
                    stock: simpleVariant ? simpleVariant.stock : '',
                    tier_prices: simpleVariant?.tier_prices?.map(tp => ({ id: tp.id, min_qty: tp.min_qty, max_qty: tp.max_qty || '', price: tp.price })) || [],
                    existingImages: p.images || [],
                    selected_options: p.options?.map(o => o.id) || [],
                    variants: p.variants?.map(v => {
                        const optionValues = {};
                        const valuesArray = v.values || v.option_values || [];
                        valuesArray.forEach(valObj => {
                            const optId = valObj.option_id || valueToOptionMap[valObj.id];
                            if (optId) optionValues[optId] = valObj.id;
                        });
                        return {
                            id: v.id, sku: v.sku, price: v.price, stock: v.stock,
                            option_values: optionValues, image: v.image,
                            tier_prices: v.tier_prices?.map(tp => ({ id: tp.id, min_qty: tp.min_qty, max_qty: tp.max_qty || '', price: tp.price })) || []
                        };
                    }) || []
                }));
            } else {
    ...
    const handleImageChange = (e) => {
        ...
        if (files.length > 0) {
            ...
            setFormData(prev => ({ ...prev, images: [...prev.images, ...files], imagePreviews: [...prev.imagePreviews, ...newPreviews] }));
        }
    };

    const removeNewImage = (index) => {
        setFormData(prev => ({ ...prev, images: prev.images.filter((_, i) => i !== index), imagePreviews: prev.imagePreviews.filter((_, i) => i !== index) }));
    };

    const deleteExistingImage = (imageId, index) => {
        setConfirmConfig({
            isOpen: true,
            title: 'حذف الصورة',
    ...
    const handleCategoryToggle = (id) => {
        setFormData(prev => ({ ...prev, categories: prev.categories.includes(id) ? prev.categories.filter(cId => cId !== id) : [...prev.categories, id] }));
    };
    ...
    // ======================
    const addVariant = () => {
        if (formData.selected_options.length === 0) { toast.error('الرجاء اختيار خيارات المنتج أولاً'); return; }
        const newSku = generateSKU({});
        setFormData(prev => ({ ...prev, variants: [...prev.variants, { sku: newSku, price: '', stock: '', option_values: {}, tier_prices: [] }] }));
    };

    const autoGenerateVariants = () => {
        if (formData.selected_options.length === 0) { toast.error('الرجاء اختيار خيارات المنتج أولاً'); return; }

        const selectedOpts = formData.selected_options.map(id => options.find(o => o.id === id)).filter(Boolean);
        const hasValues = selectedOpts.every(opt => opt.values && opt.values.length > 0);
        if (!hasValues) { toast.error('بعض الخيارات المختارة ليس لها قيم'); return; }

        const valueSets = selectedOpts.map(opt => opt.values.map(v => ({ optionId: opt.id, valueId: v.id })));
        const cartesian = (arrays) => arrays.reduce((acc, curr) => { const r = []; acc.forEach(a => { curr.forEach(b => { r.push([...a, b]); }); }); return r; }, [[]]);
        ...
        const newVariants = combinations.map((combo, idx) => {
            ...
            const ts = Date.now().toString(36).slice(-4).toUpperCase();
            return { sku: `PRD-${storeId}-${valueParts}-${ts}${idx}`, price: '', stock: '', option_values: optionValues, tier_prices: [] };
        });
    ...
    const removeVariant = (index) => {
        ...
        if (variant.id) {
            setConfirmConfig({
                isOpen: true,
                title: 'حذف المتغير',
                message: 'هل تريد حذف هذا المتغير نهائياً؟',
                variant: 'danger',
                loading: false,
                onConfirm: async () => {
                    try {
                        ...
                        toast.error(error.message || 'فشل حذف المتغير');
                    } finally {
                        setConfirmConfig(prev => ({ ...prev, loading: false }));
                    }
                }
    ...
    const validateForm = () => {
        ...
        if (!formData.has_variants) {
            ...
            if (!retailPrice || retailPrice <= 0) newErrors.price = 'يجب أن يكون السعر أكبر من 0';
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.has_variants) {
            ...
            formData.variants.forEach((variant, index) => {
                if (variant.id) data.append(`variants[${index}][id]`, variant.id);
        ...
        <form onSubmit={handleSubmit} className="space-y-8 max-w-5xl mx-auto pb-20">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">تحديث بيانات المنتج: {formData.name}</p>
                </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {formData.existingImages.map((img, index) => (
                                <div key={'exist-' + index} className="relative aspect-square rounded-xl overflow-hidden group border border-gray-200 bg-gray-50">
                                    <img src={img.url} alt="" className="w-full h-full object-cover" />
                                    {img.is_main && <div className="absolute bottom-0 left-0 right-0 bg-primary-600 text-white text-[10px] font-bold text-center py-0.5">الرئيسية</div>}
                                    <button type="button" onClick={() => deleteExistingImage(img.id, index)}
                            ...
                            {formData.imagePreviews.map((src, index) => (
                                <div key={'new-' + index} className="relative aspect-square rounded-xl overflow-hidden group border-2 border-primary-200 bg-primary-50">
                                    <img src={src} alt="" className="w-full h-full object-cover" />
                                    <div className="absolute bottom-0 left-0 right-0 bg-primary-500 text-white text-[10px] font-bold text-center py-0.5">جديدة</div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors ${formData.has_variants ? 'bg-primary-600' : 'bg-gray-200'}`}>
                        ...
                        {!formData.has_variants ? (
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-2">سعر التجزئة (ر.س)</label>
                                        <input type="number" required value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })}
                                            className={`w-full px-4 py-3 rounded-xl bg-gray-50 border-transparent focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all outline-none ${errors.price ? 'border-red-500 ring-4 ring-red-500/10 bg-white' : ''}`} />
                                    ...
                                    <div>
                                        ...
                                        <input type="number" required value={formData.stock} onChange={e => setFormData({ ...formData, stock: e.target.value })}
                                            className={`w-full px-4 py-3 rounded-xl bg-gray-50 border-transparent focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all outline-none ${errors.stock ? 'border-red-500 ring-4 ring-red-500/10 bg-white' : ''}`} />
                                        {errors.stock && <p className="text-red-500 text-xs mt-1 font-bold">{errors.stock[0] || errors.stock}</p>}
                                    </div>
                                </div>

                                {/* Tier Prices - Simple (INLINE) */}
                                <div className="border-t border-gray-100 pt-4">
                                    <div className="flex items-center justify-between mb-3">
                                        <div>
                                            <h4 className="text-sm font-bold text-gray-700">أسعار الجملة</h4>
                                            <p className="text-xs text-gray-400 mt-0.5">حدد أسعار مختلفة حسب الكمية</p>
                                        </div>
                                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, tier_prices: [...prev.tier_prices, { min_qty: '', max_qty: '', price: '' }] }))}
                                    ...
                                    <div className="space-y-2">
                                        {formData.tier_prices.map((tier, tIdx) => (
                                            <div key={tIdx} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-100">
                                                <div className="flex-1">
                                                    ...
                                                    <input type="number" min="2" placeholder="2" value={tier.min_qty}
                                                        onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.tier_prices]; arr[tIdx] = { ...arr[tIdx], min_qty: val }; return { ...prev, tier_prices: arr }; }); }}
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <input type="number" placeholder="فارغ = ∞" value={tier.max_qty}
                                                        onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.tier_prices]; arr[tIdx] = { ...arr[tIdx], max_qty: val }; return { ...prev, tier_prices: arr }; }); }}
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <input type="number" step="0.01" placeholder="0.00" value={tier.price}
                                                        onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.tier_prices]; arr[tIdx] = { ...arr[tIdx], price: val }; return { ...prev, tier_prices: arr }; }); }}
                                                ...
                                                <button type="button" onClick={() => setFormData(prev => ({ ...prev, tier_prices: prev.tier_prices.filter((_, i) => i !== tIdx) }))}
                                        ...
                                        {formData.tier_prices.length === 0 && <p className="text-xs text-gray-400 text-center py-3">لم يتم إضافة أسعار جملة بعد</p>}
                                    </div>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-6">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {options.map(option => (
                                            <label key={option.id} className={`cursor-pointer px-4 py-2 rounded-lg border text-sm font-bold transition-all ${formData.selected_options.includes(option.id) ? 'bg-primary-50 border-primary-500 text-primary-700' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                                                <input type="checkbox" className="hidden" checked={formData.selected_options.includes(option.id)}
                                                    onChange={(e) => {
                                                        if (e.target.checked) setFormData(p => ({ ...p, selected_options: [...p.selected_options, option.id] }));
                                                        else setFormData(p => ({ ...p, selected_options: p.selected_options.filter(id => id !== option.id) }));
                                                    }} />
                                                {option.name}
                                            </label>
                                        ))}
                                ...
                                <div className="space-y-4">
                                    {formData.variants.map((variant, vIdx) => (
                                        <div key={variant.id || `new-${vIdx}`} className="p-4 bg-gray-50 rounded-xl border border-gray-200 relative">
                                            <div className="flex items-center justify-between mb-3">
                                                <span className="text-xs font-bold text-gray-500">
                                                    {variant.id ? `نسخة #${variant.id}` : `نسخة جديدة #${vIdx + 1}`}
                                                    ...
                                                    return (
                                                        <div key={optId}>
                                                            ...
                                                            <select value={variant.option_values?.[optId] || ''}
                                                                onChange={(e) => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; arr[vIdx] = { ...arr[vIdx], option_values: { ...arr[vIdx].option_values, [optId]: val } }; return { ...prev, variants: arr }; }); }}
                                            ...
                                            <div className="grid grid-cols-3 gap-4 mb-3">
                                                <div>
                                                    ...
                                                    <input type="text" placeholder="SKU" required value={variant.sku}
                                                        onChange={(e) => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; arr[vIdx] = { ...arr[vIdx], sku: val }; return { ...prev, variants: arr }; }); }}
                                                ...
                                                <div>
                                                    ...
                                                    <input type="number" placeholder="السعر" required value={variant.price}
                                                        onChange={(e) => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; arr[vIdx] = { ...arr[vIdx], price: val }; return { ...prev, variants: arr }; }); }}
                                                ...
                                                <div>
                                                    ...
                                                    <input type="number" placeholder="الكمية" required value={variant.stock}
                                                        onChange={(e) => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; arr[vIdx] = { ...arr[vIdx], stock: val }; return { ...prev, variants: arr }; }); }}
                                            ...
                                            {/* Tier Prices for variant (INLINE) */}
                                            <div className="border-t border-gray-200 pt-3 mt-3">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-xs font-bold text-gray-500">أسعار الجملة</span>
                                                    <button type="button"
                                                        onClick={() => { setFormData(prev => { const arr = [...prev.variants]; const tps = arr[vIdx].tier_prices || []; arr[vIdx] = { ...arr[vIdx], tier_prices: [...tps, { min_qty: '', max_qty: '', price: '' }] }; return { ...prev, variants: arr }; }); }}
                                                ...
                                                <div className="space-y-2">
                                                    {(variant.tier_prices || []).map((tp, tpIdx) => (
                                                        <div key={tpIdx} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-100">
                                                            <div className="flex-1">
                                                                <label className="block text-[10px] font-bold text-gray-400 mb-1">من (كمية)</label>
                                                                <input type="number" min="2" placeholder="2" value={tp.min_qty}
                                                                    onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; const tps = [...(arr[vIdx].tier_prices || [])]; tps[tpIdx] = { ...tps[tpIdx], min_qty: val }; arr[vIdx] = { ...arr[vIdx], tier_prices: tps }; return { ...prev, variants: arr }; }); }}
                                                            ...
                                                            <div className="flex-1">
                                                                <label className="block text-[10px] font-bold text-gray-400 mb-1">إلى (كمية)</label>
                                                                <input type="number" placeholder="فارغ = ∞" value={tp.max_qty}
                                                                    onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; const tps = [...(arr[vIdx].tier_prices || [])]; tps[tpIdx] = { ...tps[tpIdx], max_qty: val }; arr[vIdx] = { ...arr[vIdx], tier_prices: tps }; return { ...prev, variants: arr }; }); }}
                                                                    className="w-full px-3 py-1.5 rounded-lg border border-gray-200 text-sm focus:border-primary-500 outline-none" />
                                                            </div>
                                                            <div className="flex-1">
                                                                <label className="block text-[10px] font-bold text-gray-400 mb-1">السعر (ر.س)</label>
                                                                <input type="number" step="0.01" placeholder="0.00" value={tp.price}
                                                                    onChange={e => { const val = e.target.value; setFormData(prev => { const arr = [...prev.variants]; const tps = [...(arr[vIdx].tier_prices || [])]; tps[tpIdx] = { ...tps[tpIdx], price: val }; arr[vIdx] = { ...arr[vIdx], tier_prices: tps }; return { ...prev, variants: arr }; }); }}
                                                            ...
                                                            <button type="button"
                                                                onClick={() => { setFormData(prev => { const arr = [...prev.variants]; const tps = (arr[vIdx].tier_prices || []).filter((_, i) => i !== tpIdx); arr[vIdx] = { ...arr[vIdx], tier_prices: tps }; return { ...prev, variants: arr }; }); }}
                                                                className="mt-4 p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
                                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                            </button>
                                                        </div>
